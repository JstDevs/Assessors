...
interface ImprovementUnitValue {
    value_id: number;
    item_id: number;
    improvement_name?: string; // Joined from backend
    unit_value: number;
...
const ConfigDialog: React.FC<ConfigDialogProps> = ({ isOpen, onClose, type, mode, data, onSuccess }) => {
    ...
    // For Values dropdown
    const [itemOptions, setItemOptions] = useState<LandImprovementItem[]>([]);
    ...
    const fetchItemOptions = async () => {
        ...
        try {
            // Fetch all items to populate dropdown
            const res = await api.get('loi/'); 
            const list = res.data.data ?? res.data;

            const uvRes = await api.get('loi/uv/'); 
            const existingUVs = uvRes.data.data ?? uvRes.data;
            // For now, let's just show all active items
            if (Array.isArray(list) && Array.isArray(existingUVs)) {
                const availableItems = list.filter((item: LandImprovementItem) => {
                    // Check if item already has a unit value
                    const isTaken = existingUVs.some((uv: ImprovementUnitValue) => uv.improvement_id === item.improvement_id);
                    // If we are editing, allow the current item to be selected (it's "taken" by itself)
                    const isSelf = mode !== 'ADD' && item.improvement_id === data?.improvement_id;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (type === 'ITEMS') {
                // Routes: li/ (POST), li/:id (PUT)
                if (mode === 'ADD') {
                    await api.post('loi', formData);
                } else if (mode === 'EDIT') {
                    await api.put(`loi/${formData.improvement_id}`, formData);
                }
            } else {
                // Routes: li/uv (POST), li/uv/:id (PUT)
                const url = 'loi/uv'; 
                if (mode === 'ADD') {
    ...
    const titleMap = {
        'ITEMS': 'Land Improvement Item',
        'VALUES': 'Improvement Unit Value'
    };
            ...
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh] border border-slate-200 overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/80 backdrop-blur-md">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${type === 'ITEMS' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                            {type === 'ITEMS' ? <Trees size={20} /> : <Calculator size={20} />}
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-white">
                    ...
                    <form id="config-form" onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {type === 'ITEMS' && (
                            <>
                                <div className="space-y-1">
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide">Improvement Name <span className="text-red-500">*</span></label>
                                    <input type="text" name="improvement_name" value={formData.improvement_name || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="e.g. Mango Tree" />
                                </div>
                                ...
                                <div className="flex items-center gap-3 pt-2">
                                    <div className="relative flex items-center">
                                        <input 
                                            ...
                                            disabled={isReadOnly}
                                            className="peer h-5 w-5 cursor-pointer appearance-none rounded-md border border-slate-300 transition-all checked:border-emerald-600 checked:bg-emerald-600 focus:ring-2 focus:ring-emerald-500/20" 
                                        />
                        ...
                        {type === 'VALUES' && (
                            <>
                                <div className="space-y-1">
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide">Select Improvement <span className="text-red-500">*</span></label>
                                    <select 
                                        name="improvement_id" 
                                        value={formData.improvement_id || ''} 
                                        onChange={handleChange} 
                                        disabled={isReadOnly || loadingOptions || mode === 'EDIT'} 
                                        required 
                                        className="form-input disabled:bg-slate-100 disabled:text-slate-500"
                                    >
                                        <option value="">{loadingOptions ? 'Loading items...' : '-- Select Item --'}</option>
                                        {itemOptions.map(item => (
                                            <option key={item.improvement_id} value={item.improvement_id}>{item.improvement_name}</option>
                                        ))}
                                    </select>
                                    {mode === 'ADD' && itemOptions.length === 0 && !loadingOptions && (
                                        <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
                                            <AlertCircle size={12} /> All active improvements already have unit values assigned.
                                        </p>
                                    )}
                                </div>
...
export default function LandImprovements() {
    ...
    const filteredData = Array.isArray(data) ? data.filter(item => {
        ...
        if (activeTab === 'ITEMS') {
            return item.improvement_name?.toLowerCase().includes(search) || item.description?.toLowerCase().includes(search);
        } else {
            return (item.improvement_name || '').toLowerCase().includes(search);
        }
    }) : [];
            ...
            <div className="w-full max-w-full space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                            <Trees className="text-emerald-600" /> Land Improvements
                        </h1>
                        <p className="text-slate-500 text-sm mt-1">Manage improvement items (plants, trees, fences) and their unit values.</p>
                    </div>
                    <button 
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[600px]">
                    ...
                    <div className="flex border-b border-slate-200 px-2 overflow-x-auto">
                        <TabButton id="ITEMS" label="Improvement Items" icon={Package} />
                    ...
                    <div className="p-4 border-b border-slate-100 flex gap-4 bg-slate-50/30 sticky top-[49px] z-20">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <input 
                                type="text" 
                                placeholder={activeTab === 'ITEMS' ? "Search improvements..." : "Search values..."}
                                value={searchTerm}
                    ...
                    {/* Content Table */}
                    <div className="flex-1 overflow-auto w-full max-h-[500px]">
                        {loading ? (
                                    ...
                                    <tr>
                                        {activeTab === 'ITEMS' ? (
                                            <>
                                                ...
                                                <th className="px-6 py-4">Improvement Name</th>
                                    ...
                                    {filteredData && filteredData.length > 0 ? (
                                        filteredData.map((item, idx) => (
                                            <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                                                {activeTab === 'ITEMS' ? (
                                                    <>
                                                        <td className="px-6 py-4 font-mono text-slate-500 text-xs">{String(item.improvement_id).padStart(3, '0')}</td>
                                                        <td className="px-6 py-4 font-bold text-slate-800">{item.improvement_name}</td>
                                                    ...
                                                    <>
                                                        ...
                                                        <td className="px-6 py-4 font-bold text-slate-800 flex items-center gap-2">
                                                            ...
                                                            {item.improvement_name || `Item #${item.item_id}`}
                                                        </td>
