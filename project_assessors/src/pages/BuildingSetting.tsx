...
type TabType = 'KINDS' | 'STRUCTURAL' | 'ACTUAL_USE';
...
const ConfigDialog: React.FC<ConfigDialogProps> = ({ isOpen, onClose, type, mode, data, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const endpointMap = {
                'KINDS': 'building-kind',
                'STRUCTURAL': 'structural-type',
                'ACTUAL_USE': 'building-actual-use'
            };
            ...
            if (mode === 'ADD') {
                ...
                const id = type === 'KINDS' ? formData.bk_id : type === 'STRUCTURAL' ? formData.st_id : formData.bau_id;
                    ...
                    <form id="config-form" onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="code" value={formData.code || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="e.g. RES" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-600 mb-1">PC ID (Class) <span className="text-red-500">*</span></label>
                                        <input type="number" name="pc_id" value={formData.pc_id || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="ID" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Name <span className="text-red-500">*</span></label>
                                    <input type="text" name="name" value={formData.name || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="e.g. Residential" />
                                </div>
                                <div>
                        ...
                        {type === 'STRUCTURAL' && (
                            <>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Code <span className="text-red-500">*</span></label>
                                    <input type="text" name="code" value={formData.code || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="e.g. IIA" />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Name <span className="text-red-500">*</span></label>
                                    <input type="text" name="name" value={formData.name || ''} onChange={handleChange} disabled={isReadOnly} required className="form-input" placeholder="e.g. Wood/G.I. Sheets" />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Description</label>
                                    <textarea name="description" value={formData.description || ''} onChange={handleChange} disabled={isReadOnly} rows={3} className="form-input" />
...
export default function BuildingSettings() {
    const [activeTab, setActiveTab] = useState<TabType>('KINDS');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const endpointMap = {
                    'KINDS': 'bkmt/bklist', 
                    'STRUCTURAL': 'smv/stlist', 
                    'ACTUAL_USE': 'bkmt/baulist'
                };
        ...
        <div className="min-h-screen bg-slate-50/50 p-6 md:p-8 font-sans w-full text-slate-900">
            <div className="w-full max-w-full space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Building Configuration</h1>
                        <p className="text-slate-500 text-sm mt-1">Manage classifications, structural types, and actual use codes.</p>
                    </div>
                    <button 
                    ...
                    >
                        ...
                        Add New {activeTab === 'KINDS' ? 'Kind' : activeTab === 'STRUCTURAL' ? 'Type' : 'Actual Use'}
                    </button>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                    ...
                    <div className="flex border-b border-slate-200 px-2 overflow-x-auto">
                        <TabButton id="KINDS" label="Building Kinds" icon={Building} />
                        <TabButton id="STRUCTURAL" label="Structural Types" icon={Hammer} />
